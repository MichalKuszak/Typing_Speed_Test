...
class Main(ttk.Frame):
    def __init__(self, parent):
        ...
        self.title_label.grid(row=0, column=0, columnspan=2, padx=5, pady=5, sticky="w")
        ...
        self.description_label.grid(row=1, column=0, columnspan=2, padx=5, pady=5, sticky="w")
        ...
        self.user_entry.bind("<KeyRelease>", self.start)
        ...
        self.reset_button = ttk.Button(self, text="Reset", bootstyle='warning', command=self.reset)
    ...
    def start(self, event):
        if not self.running:
            if not event.keycode in FUNC_KEYS:
                ...
                t.start()
        if not self.test_text_label.cget('text').startswith(self.entry_var.get()):
            self.user_entry.config(foreground='red')
        else:
            self.user_entry.config(foreground='black')
        if self.entry_var.get() == self.test_text_label.cget('text'):
            self.running = False
            self.user_entry.config(foreground='green')
    ...
    def time_thread(self):
        while self.running:
            time.sleep(0.1)
            self.counter += 0.1
            chars_per_second = len(self.entry_var.get()) / self.counter
            chars_per_minute = chars_per_second * 60
            words_per_second = len(self.entry_var.get().split(" ")) / self.counter
            words_per_minute = words_per_second * 60
            self.info_label.config(text=f"Your typing speed: \n{chars_per_second:.2f} CPS\t\t{words_per_second:.2f}WPS\n"
                                               f"{chars_per_minute:.2f} CPM\t\t{words_per_minute:.2f} WPM")

    def reset(self):
        self.running = False
        self.counter = 0
        self.info_label.config(text="Your typing speed: \n0.00 CPS\t\t0.00 WPS\n"
                                               "0.00 CPM\t\t0.00 WPM")
        self.test_text_label.config(text=word_picker())
...
if __name__ == "__main__":
    app = GUI("Typing Speed Test", (800, 600))
    app.mainloop()
